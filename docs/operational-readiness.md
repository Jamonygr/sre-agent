...
| Cost control | Low-cost profile, disabled premium services by default, budget alerting, scheduled VM start/stop | `environments/cheap-lab.tfvars`, `modules/cost-management`, `enable_scheduled_startstop` |
...
4. Run `.\scripts\Invoke-QualityGate.ps1 -PlanProfiles cheap-lab,lab`.
...
- Start with `cheap-lab` for review. Move to `lab` only when app-platform services are needed.
